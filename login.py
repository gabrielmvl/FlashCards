...
Dados = {}
FernetKey = open('testes/FernetKey.txt', 'rb').read()
f = Fernet(FernetKey)
...
def BancoDeDados():
    ...
    with open('testes/Usuarios.csv', 'r', encoding='utf-8') as file:
        BancoDeDados = csv.reader(file, delimiter=';')
...
def Registro():
    ...
    with open('testes/Usuarios.csv', 'a', encoding='utf-8') as file:
        csv.writer(file, delimiter=';').writerow([Crypt_Email.decode(), Crypt_UserName.decode(), Crypt_Senha.decode(), id])

def Login():
    Email, Senha = ColetarDados('Login')
    with open('testes/FernetKey.txt', 'rb') as f:
        file = f.read
        Emails = Dados.keys()
        EmailsDecrypt = []
        for mail in Emails:
            EmailsDecrypt.append(f.decrypt(mail))
        if Email in EmailsDecrypt():
            SenhaHash = Dados[f.encrypt(Email)][1]
            bcrypt.checkpw(Senha, SenhaHash)
